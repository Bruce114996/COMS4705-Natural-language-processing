...
class FeatureExtractor(object):
    ...
    def get_input_representation(self, words, pos, state):
        ...
        word_idx = []
        stacks, buffer = state.stack.copy(), state.buffer.copy()
        i, j=0, 0
        while stacks and i<3:
            idx=stacks.pop()
            i+=1
            if not words[idx] and len(stacks)>0:
                word_idx.append(4)
                continue
            if not words[idx] and len(stacks)==0:
                word_idx.append(self.word_vocab["<ROOT>"])
                continue
            if pos[idx]=='NNP' or pos[idx]=='CD':
                word_idx.append(self.word_vocab["<{}>".format(pos[idx])])
                continue
            try:
                word_idx.append(self.word_vocab[words[idx]])
            except KeyError:
                word_idx.append(self.word_vocab["<UNK>"])
        while i<3:
           word_idx.append(4)
           i+=1

        while buffer and j<3:
            idx=buffer.pop()
            j+=1
            if not words[idx] and len(buffer)>0:
                word_idx.append(4)
                continue
            if not words[idx] and len(buffer)==0:
                word_idx.append(self.word_vocab["<ROOT>"])
    ...
    def get_output_representation(self, output_pair):  
        ...
        return tf.keras.utils.to_categorical(result, num_classes=91)
